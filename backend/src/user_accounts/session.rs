...
pub async fn get_session_from_id(
    db_pool: &Pool<Postgres>,
    session_id: &[u8],
) -> Result<Session, BackendError> {
    let query = sqlx::query_file_as!(
        Session,
        "src/user_accounts/sql/retrieve_session.sql",
        session_id
    );
    query.fetch_one(db_pool).await.map_err(|e| {
...
pub fn extract_session_id_from_cookie(cookies: Cookies) -> Result<Vec<u8>, BackendError> {
    ...
    hex::decode(session_id_hex)
        .map_err(|_| BackendError::NotAuthorized("Session id is corrupted".to_string()))
}
