...
*/
pub async fn create_user(app_state: State<Arc<AppState>>, Json(new_user): Json<UserLoginDetails>) -> Result<StatusCode, BackendError> {
    
    let new_user_email = new_user.email.clone();
    let new_user_email = new_user_email.ok_or_else(|| {
        BackendError::UnprocessableEntity("Unprocessable entity".to_string())
    })?;


    if new_user.username.is_empty() || new_user_email.is_empty() || new_user.password.is_empty() {
        return Err(BackendError::UnprocessableEntity("All fields need to be submitted".to_string()));
    }
    ...
    let password_hash_to_store = Argon2::hash_password(&app_state.argon_ctx,new_user.password.as_bytes(),&salt).map_err(|e| {
    ...
    let email_encrypted = crypto_with_user_key.encrypt(&new_user_email.as_bytes()).map_err(|e| {
    ...
    //encrypt user key 
    let user_key_encrypted = crypto_with_master_key.encrypt(&crypto_with_user_key.key()).map_err(|e| {
    ...
    let query = sqlx::query_file_as!(UserRow, "src/user_accounts/sql/insert_user.sql",
            ...
            match err.as_database_error() {
                Some(err) => {
                    ...
                    if err.code() == Some("23505".into()) {
                        return Err(BackendError::Conflict("Username or email already exists".to_string()));
                    }
                    return Err(BackendError::InternalServerError("Unknown error".to_string()));
                }
                None => {
...
pub async fn find_all_users(app_state: State<Arc<AppState>>) -> Result< Json<Vec<UserPublic>>, BackendError>{
    let mut all_users :Vec<UserPublic> = Vec::new(); 

    let mut query_result = sqlx::query("SELECT username FROM useraccounts;")
    .fetch(&app_state.db_pool);

    while let Some(row_result) = query_result.next().await{
        match row_result {
            Ok(row) => {
                all_users.push(UserPublic { username: row.get("username") });
            },
            Err(err) => {
                match err.as_database_error() {
                    Some(err) => {
                        logging::log_users(format!("Error: {}", err).as_str());
                        return Err(BackendError::InternalServerError("Internal server error".to_string()))
                    }
                    None => {
...
*/
pub async fn user_login( app_state: State<Arc<AppState>>, cookies: Cookies, Json(user_login_details): Json<UserLoginDetails>) -> Result< StatusCode , BackendError>{

    //verify user START
    if user_login_details.username.is_empty() || user_login_details.password.is_empty()  {
        return Err(BackendError::UnprocessableEntity("All fields must contain a value".to_string()))
    }

    let user = get_user_from_username(&user_login_details.username, &app_state.db_pool).await.map_err(|_e| {
        BackendError::NotFound("User not found".to_string())
    })?;


    let found_hash = PasswordHash::new(&user.password_hash).map_err(|_| {
        BackendError::InternalServerError("Internal Server error".to_string())
    })?;
    
    Argon2::verify_password(&app_state.argon_ctx, user_login_details.password.as_bytes(), &found_hash).map_err( |_| {
        BackendError::NotAuthorized("Password does not match".to_string())
    })?;
    //verify user END
    ...
    let query = sqlx::query_file!("src/user_accounts/sql/insert_session.sql", &session_id, user.id, expires)
        .execute(&app_state.db_pool)
        .await;
    ...
    match query {
        ...
        Err(e) =>{
            ...
            return Err(BackendError::InternalServerError("Session creation failed".to_string()));
        }
    }
...
fn decrypt_user_data(app_state: State<Arc<AppState>>, user : &mut User) -> Result< &mut User, BackendError> {
...
async fn get_user_from_session_cookie(db_pool: &Pool<Postgres>, cookies: Cookies) -> Result< User , BackendError>{
    ...
    get_user_from_id(&user_session.user_id,&db_pool)
        .await
        .map_err(|e| {
            BackendError::InternalServerError(format!("Could not retrieve user: {}", e))
        })
}

pub async fn get_private_user(app_state: State<Arc<AppState>>, cookies: Cookies, req: Request<Body>) -> Result< Json<UserPrivate> , BackendError>{
    ...
    let user_email = String::from_utf8(user.email.clone())
        .map_err(|e| {
            BackendError::InternalServerError(format!("{}",e.to_string()))
        })?;
    ...
    let user_agent = parts.headers
        .get("user-agent")
        .unwrap_or(
            &HeaderValue::from_static("user agent not found")
        )
        .to_str()
        .map_err(|e| {
            BackendError::InternalServerError(format!("Failed to parse user agent: {}", e.to_string()))
        })?
        .to_string();
...
pub async fn user_logout(app_state: State<Arc<AppState>>, cookies:  Cookies ) -> Result< StatusCode , BackendError>{
    ...
    let session_id_bytes = hex::decode(session_id_hex).map_err(|_| {
        BackendError::InternalServerError("Error logging out".to_string())
    })?;
    ...
    sqlx::query_file!("src/user_accounts/sql/revoke_session.sql", session_id_bytes)
        ...
        .await
        .map_err(|_| {
            BackendError::InternalServerError("Unable to logout".to_string())
        })?;
...
async fn get_user_from_username(username_to_search_for: &str, db_pool: &Pool<Postgres>) -> Result<User, String> {

    let query = sqlx::query_as!(User,"SELECT * FROM useraccounts WHERE username=$1 LIMIT 1;", username_to_search_for)
    .fetch_one(db_pool)
    .await;
...
async fn get_user_from_id(user_id: &i32, db_pool: &Pool<Postgres>) -> Result<User, String> {
    ...
    let query = sqlx::query_as!(User,"SELECT * FROM useraccounts WHERE id=$1 LIMIT 1;", user_id)
    .fetch_one(db_pool)
    .await;
...
*/
pub async fn find_user(app_state: State<Arc<AppState>>, path: Path<String>) -> Result<Json<Vec<UserPublic>>, BackendError>{
    let username_to_search_for = path.0;
    // println!("looking for user: {}", username_to_search_for);

    let mut query_found_users = sqlx::query_file!("src/user_accounts/sql/find_users.sql", username_to_search_for ) .fetch(&app_state.db_pool);
