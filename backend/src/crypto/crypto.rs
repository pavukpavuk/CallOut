use aes_gcm::{
    Aes256Gcm, Key, KeySizeUser, Nonce, aead::{Aead, AeadCore, KeyInit, OsRng, }, // Or `Aes128Gcm`
};
use hex;
...
impl DecryptedData {
    ...
    pub fn as_utf8(&self) -> Result<String, String>{
        String::from_utf8(self.decrypted_ciphertext.clone()).map_err(|e| {
            e.to_string()
        })
    }
...
impl Crypto{
    ...
    pub fn encrypt(&self, input: &[u8]) -> Result<EncryptedData, String>{
        ...
        match self.cipher.encrypt(&nonce, input) {
            ...
            Err(_) => { // purposely obtuse errors so no need to propogate
